...
#print(metro_all)
# dataset => data.go.kr, seoul.data.go.kr(korquad.github.io)
"""
...
#print(df)
df['평균 승차인원수']=metro_get_on.mean(axis=1).astype(int)
#print(df['평균 승차인원수'])
top10_on=df.sort_values(by="평균 승차인원수",ascending=False).head(10)
plt.figure(figsize=(10,8))
plt.rc('font',family="Malgun Gothic")
plt.rcParams['axes.unicode_minus']=False
plt.bar(top10_on.index,top10_on['평균 승차인원수'])
plt.show()
...
df=pd.DataFrame(data=metro_st_line2,index=metro_st_line2['지하철역'])
df['평균 하차인원수']=metro_get_off.mean(axis=1).astype(int)
#print(df['평균 하차인원수'])
top10_off=df.sort_values(by="평균 하차인원수",ascending=False).head(10)
plt.figure(figsize=(10,8))
plt.rc('font',family="Malgun Gothic")
plt.rcParams['axes.unicode_minus']=False
plt.bar(top10_off.index,top10_off['평균 하차인원수'])
plt.show()